...
export const voluntarySchema = z.object({
    ...
    email: z.string().nonempty({ message: "O e-mail é obrigatório!" }).email({ message: "Por favor, insira um endereço de email válido" }),
    phone: z.string().nonempty({ message: "O telefone é obrigatório!" }).regex(phoneRegex, {message: "Por favor, insira um telefone válido!"}),
